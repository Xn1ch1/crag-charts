 ...
 * @return {string} Returns formatted string
 */
export function formatLabel(value, type = 'number', currencySymbol = 'GBP', decimalPlaces = 0, locale = 'en-GB') {
...
export function calculateScale(min, max, base) {
...
export function createSVGChartArea() {
...
export class VAxis extends CragCore {
...
export class HAxis extends CragCore {
...
export class ToolTip extends CragCore {
...
export class Title extends CragCore {
...
/**
 * Attach CSS files to <head> relative to the current module
 * @param {...string} cssFiles CSS file names to attach
 */
export function attachCssToHead(...cssFiles) {
    ...
    // Add CragCharts.css by default
    const files = ['CragCharts.css', ...cssFiles];

    // Compute base path automatically from module URL
    const moduleUrl = new URL(import.meta.url);
    // Remove the module file name to get the folder (e.g., /dist/2.0.0/)
    const basePath = moduleUrl.href.replace(/\/[^/]+\.js$/, '/');

    files.forEach(file => {
        if (document.getElementById(`cragcharts-css-${file}`)) {
            // CSS file already attached
            return;
        }
